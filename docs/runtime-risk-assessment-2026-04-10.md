...
- [x] P0.3 crash-idempotent 防重：新增 run 级 `checkpoints/` WAL 与 per-execution `execution-outcome.json` durable marker；role 执行先提交 execution outcome，再由 graph loop 发出 checkpoint，resume 会先重放 pending checkpoint，并对缺失 checkpoint 或缺失 reconciliation metadata 的 outcome 做补偿回填，避免重复执行。
