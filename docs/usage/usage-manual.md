...
1. `README.md`
2. `docs/README.md`
3. `docs/usage/product-introduction.md`
4. 本手册
5. `docs/usage/ogsystem-orchestration-semantics-v1.md`
6. `docs/development/DECISIONS.md`
7. `docs/usage/ogsystem-ebook.md`
...
- user-level configuration for the selected CLI backend
- `system.mmd`
- 本地最小 `og-roles/`
...
OGS 使用项目 `.ogs/model-selection.json` 选择 backend 与 modelId，并调用对应 CLI 提供的持久服务。凭据由各 CLI 自己读取其用户配置；OGS 不导入、不同步，也不托管凭据。
...
- default Agent execution path: set `backend` and `modelId` in `.ogs/model-selection.json`
...
- `role_input` contracts validate the projected `context.map` object before prompt rendering; they do not replace the built-in runtime prompt-input schema
...
- `system.mmd`: role graph, events, law binding, and handoff structure
...
`.ogs/model-selection.json` is the single model-binding source. Configure a backend and its model ID for project defaults or individual roles. `system.mmd` describes roles and handoffs only.

`.ogs/model-catalog.json` is generated by `ogs models discover` and contains installed CLI backends and runnable models. `ogs models sync` refreshes discovery and creates the selection file only when it is missing. OGS invokes persistent CLI services and uses each CLI's existing user configuration; credentials are not imported or managed by OGS. OpenCode uses `opencode serve`; Codex uses `codex app-server --stdio`. Other detected CLIs remain unavailable for runs until a persistent adapter is supported.
...
- `.ogs/model-selection.json` is the runtime authority for project defaults and per-role backend/model overrides
...
- `ogs models discover` refreshes the local CLI/model catalog; `ogs models sync` also initializes a missing selection file
- keep model choices out of `system.mmd`; use Studio or `.ogs/model-selection.json` to change a role's backend/model
- model roles run through their selected persistent CLI service:
  ...
  - output is normalized to the role's validated JSON output contract
  - backend protocol failures retain backend-specific diagnostics where available
  - the selected backend and model are included in run/session audit metadata
...
OGS does not load a separate credential file. Configure authentication through the selected CLI's supported user-level setup; OGS starts that CLI service and leaves credential ownership with it.
...
- run-level persistent service metadata is stored under `.ogs/runs/<run-id>/`; backend-specific details are recorded with the run artifacts
...
- `sessionLineageId` scopes backend session/thread reuse and sibling-branch isolation
...
OpenCode persistent-service behavior:
...
Codex persistent-service behavior:

- one OGSystem run starts one `codex app-server --stdio`
- each role branch starts or resumes a persistent thread on that server
- session metadata is retained for audit and run resume; the app-server closes with the run
