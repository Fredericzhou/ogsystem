...
- [x] Remove `Legacy Tabs` from the primary lifecycle navigation.
...
- [x] Reorganize Operate from stacked legacy panels into a run-first workspace.
...
- [x] Add explicit Build modes: Edit, Dry Run, and Debug.
...
- [x] Clarify Build persistence actions and state.
...
- [x] Tighten Validate & Release gates.
...
- [x] Make Project a true lifecycle starting point.
...
- [x] Contextualize top-level actions.
...
- [x] Remove duplicated Config exposure.
...
- [x] Refine desktop Studio layout.
...
- [x] Productize remaining hard-coded messages.
...
- Project Wizard is implemented as a visual lifecycle start panel over existing Studio templates, current-project role packages, model/profile summaries, entry role state, and explicit project load. It does not add hidden project-creation writes or a new backend contract.
- Legacy panel rendering remains available only through explicit `?lifecycle=legacy` fallback; default navigation exposes the four lifecycle entries.
- Build dry run now validates, generates Mermaid, saves, then opens the run form, and returns dry-run results to Build Debug with an explicit Open in Operate action.
